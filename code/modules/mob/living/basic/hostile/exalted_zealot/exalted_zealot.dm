...
'abilities' L+veil shift + speedup = 2,3
			...
			+drake ability but lifesteal runes = 3
			L+shotgun BLAST = 1,2,3
...
/datum/ai_behavior/exalted/shotgun_blast/setup(datum/ai_controller/controller, target_key)
	. = ..()
	if(!ishuman(controller.blackboard[target_key])) //lock in...
		return FALSE
...
/datum/ai_behavior/exalted/shotgun_blast/finish_action(datum/ai_controller/controller, succeeded, target_key, targeting_strategy_key, hiding_location_key)
	. = ..()
